...
# balena-block-experiments (BBE) development, which consists 
...
# The freshly built BBE SDK will sit within the _work subdirectory 
# of the repository, which is masked from addition to version control
# by the .gitignore file.

sdk_date=$(date +%y%m%d)
for s in a b c d e f g h i j k l m n o p
do
    sdk_reldir=./_work/sdk.$sdk_date$s
    if [ -d $sdk_reldir ]
    then
        continue
    fi
...
balena_version=v25.1.9
jdk21_version=21.0.11
jdk25_version=25.0.3
android_cltools_version=15859902

osname=$(uname -s)
if [ "$osname" = "Linux" ]
then
  jdk21_url=https://download.oracle.com/java/21/archive/jdk-${jdk21_version}_linux-x64_bin.tar.gz
  jdk25_url=https://download.oracle.com/java/25/archive/jdk-${jdk25_version}_linux-x64_bin.tar.gz
...
then
  ...
  jdk21_url=https://download.oracle.com/java/21/archive/jdk-${jdk21_version}_macos-x64_bin.tar.gz
  jdk25_url=https://download.oracle.com/java/25/archive/jdk-${jdk25_version}_macos-x64_bin.tar.gz
...
set_java_home jdk-$jdk21_version
